...
1. A URL to the app's repo, `app_repo_url` (for private repos, provide `auth_user` and `auth_pw`). This should be the full URL like, https://github.com/Dripdrop12/RInnoApp.
2. A release tag on GitHub, or a version on BitBucket. If you forget to create a release, your app will error when it calls `get_remote_version` and `parsed_response`'s subscript will be "out of bounds" because the list will be empty.
3. An R package structure that can be installed via `devtools::install_github` or `devtools::install_bitbucket`.
4. Add your app to the `pkgs` argument of `create_app`
